(function(){
  ...
  // Rounds-only filters: deliberately not shared with Bolus, where a hidden
  // ward could conceal an overdue administration.
  let roundsWardFilter = 'all';
  let roundsModeFilter = 'all';
  let bolusWardFilter = 'all';
  let bolusDateFilter = '';
  ...
  // Compact clinical-day caption for the Day 0–5 tabs.
  ...
  // Rounds shows the latest clinically recorded, non-empty review remark.
  // This deliberately follows the Day 0–5 order rather than the save time:
  // a later APS day is the latest clinical detail even if an earlier day was
  // filled in afterwards. Empty saved reviews do not create a Rounds message.
  function latestReviewGlimpse(p){
    const noteFor = holder=>{
      for(let d=5; d>=0; d--){
        const dObj = getDayRaw(holder, d);
        if(dObj.remarks && dObj.remarks.trim()) return dObj.remarks.trim();
      }
      return '';
    };
    if(p.mode==='NBC'){
      const notes = activeCatheters(p).map((c, i)=>{
        const note = noteFor(c);
        if(!note) return '';
        const label = (c.label&&c.label.trim()) || (p.location&&p.location.trim()) || (activeCatheters(p).length>1 ? `Catheter ${i+1}` : '');
  ...
  function getDayRaw(holder, idx){
    ...
    if(v && typeof v === 'object') return {
      ...
      bolusEvents: Array.isArray(v.bolusEvents) ? v.bolusEvents : [],
      scheduleChanges: Array.isArray(v.scheduleChanges) ? v.scheduleChanges : []
    };
    return {stamp:null, painRest:'',painMove:'',motorBlock:'',dermCover:'',catheterSite:'',remarks:v||'', drug:null, frequency:null, freqType:null, startedAt:null, bolusEvents:[], scheduleChanges:[]};
  }
  ...
  function dosingScheduleSelect(id, freqType){
    return `<select id="${id}">
      ...
      <option value="PIB Pump" ${freqType==='PIB Pump'?'selected':''}>PIB Pump (legacy)</option>
  ...
  function renderDoseAndAssessmentFields(suf, eff, dObj, activeDay, holder){
    ...
    return `
      ...
      ${renderDayDoseActivity(dObj)}
    `;
  }
  // Bolus-board actions are also visible in the latest daily detail, without
  // forcing clinicians to re-enter the same information in the review form.
  function renderDayDoseActivity(dObj){
    const events = dObj.bolusEvents || [];
    const changes = dObj.scheduleChanges || [];
    if(!events.length && !changes.length) return '';
    const rows = [
      ...changes.map(e=>`<div>Schedule changed: ${escapeHtml(e.from||'—')} → <b>${escapeHtml(e.to||'—')}</b> · ${escapeHtml(fmtTimeOfDay(e.ts))} · ${escapeHtml(e.by||'Unknown')}</div>`),
      ...events.map(e=>`<div>Bolus logged · ${escapeHtml(fmtTimeOfDay(e.ts))} · ${escapeHtml(e.by||'Unknown')}</div>`)
    ];
    return `<div class="day-dose-activity"><label>Bolus activity</label>${rows.join('')}</div>`;
  }
  ...
  function fmtDDMMYYFromTs(ts){
    if(!ts) return '';
    const iso = new Date(ts).toLocaleDateString('en-CA', {timeZone:HOSPITAL_TZ});
  ...
  // TID intentionally shares Q8H's rolling eight-hour timing while retaining
  // its distinct clinical label everywhere in the interface and audit trail.
  const BOLUS_INTERVAL_HOURS = {'Q4H':4, 'Q6H':6, 'Q8H':8, 'TID':8, 'Q12H':12};
  ...
  function lastBolusAtRaw(holder){
    // A schedule change starts a fresh interval. Once a later bolus is
    // recorded, that bolus naturally becomes the new interval anchor.
    const lastGiven = holder.boluses && holder.boluses.length ? holder.boluses[holder.boluses.length-1].ts : null;
    const changedAt = holder.scheduleChangedAt || null;
    if(lastGiven || changedAt) return Math.max(lastGiven||0, changedAt||0);
  ...
  function hospitalTimeNow(){
    return new Date().toLocaleTimeString('en-GB', {timeZone:HOSPITAL_TZ, hour:'2-digit', minute:'2-digit', hour12:false});
  }
  // The most recently documented clinical day is the right place to surface
  // quick Bolus-board activity. If no review exists yet, retain it on Day 0
  // so it is never lost and becomes visible as soon as that detail is opened.
  function latestDetailDayIndex(holder){
    for(let d=5; d>=0; d--) if(getDayRaw(holder,d).stamp) return d;
    return 0;
  }
  function addDayActivity(holder, type, entry){
    const day = latestDetailDayIndex(holder);
    const current = getDayRaw(holder, day);
    const field = type === 'bolus' ? 'bolusEvents' : 'scheduleChanges';
    holder['day'+day] = {...current, [field]: [...(current[field]||[]), entry]};
  }
  ...
  // a warning before.
  let dcResolve = null;
  function askConfirm({title, message, confirmLabel, danger, showDate, dateValue}){
    return new Promise(resolve=>{
      ...
      $('dcDateRow').style.display = showDate ? '' : 'none';
      if(showDate){
        $('dcDateInput').max = todayStr();
        $('dcDateInput').value = dateValue || todayStr();
      }
  ...
  // ---------- Bolus schedule popup ----------
  let bolusScheduleResolve = null;
  function askBolusScheduleChange({name, current}){
    return new Promise(resolve=>{
      bolusScheduleResolve = resolve;
      $('bolusScheduleName').textContent = name;
      $('bolusScheduleSelect').value = current || '';
      $('bolusScheduleBackdrop').classList.add('open');
    });
  }
  function closeBolusScheduleModal(value){
    $('bolusScheduleBackdrop').classList.remove('open');
    if(bolusScheduleResolve){ const r = bolusScheduleResolve; bolusScheduleResolve = null; r(value); }
  }
  $('bolusScheduleCancel').addEventListener('click', ()=>closeBolusScheduleModal(null));
  $('bolusScheduleSave').addEventListener('click', ()=>closeBolusScheduleModal($('bolusScheduleSelect').value));
  $('bolusScheduleBackdrop').addEventListener('click', e=>{ if(e.target.id==='bolusScheduleBackdrop') closeBolusScheduleModal(null); });
  ...
  async function openDischargeConfirm(id){
    ...
    const ok = await askConfirm({
      ...
      showDate: true,
      dateValue: p.dischargedDate || todayStr()
    });
    if(!ok) return;
    const dischargeDate = $('dcDateInput').value;
    if(!dischargeDate){ toast('Choose a discharge date before continuing', 4000); return; }
    if(dischargeDate > todayStr()){ toast('Discharge date cannot be in the future', 4000); return; }
    try{
      await savePatientField(id, (base)=>{
        base.status = 'discharged';
        // Noon avoids timezone rollover while retaining a numeric timestamp
        // for legacy sorts and duration analytics.
        base.dischargedDate = dischargeDate;
        base.dischargedAt = new Date(dischargeDate+'T12:00:00').getTime();
        logActivity(base, 'discharged', {dischargedDate});
      });
    }catch(e){
  ...
  function renderRounds(){
    ...
    const wards = [...new Set(active.map(p=>(p.ward||'Unassigned').trim() || 'Unassigned'))].sort((a,b)=>a.localeCompare(b));
    if(roundsWardFilter!=='all' && !wards.includes(roundsWardFilter)) roundsWardFilter = 'all';
    const wardSelect = $('roundsWardFilter');
    const modeSelect = $('roundsModeFilter');
    wardSelect.innerHTML = `<option value="all">All wards</option>${wards.map(w=>`<option value="${escapeHtml(w)}">${escapeHtml(w)}</option>`).join('')}`;
    wardSelect.value = roundsWardFilter;
    modeSelect.value = roundsModeFilter;
    const filtered = active.filter(p=>
      (roundsWardFilter==='all' || ((p.ward||'Unassigned').trim() || 'Unassigned')===roundsWardFilter) &&
      (roundsModeFilter==='all' || (p.mode||'Epidural')===roundsModeFilter)
    );
    $('roundsCount').textContent = filtered.length;
    ...
    const {wardNames, groups} = groupByWard(filtered);
    if(!wardNames.length){
      const emptyText = active.length ? 'No patients match the selected filters.' : 'No active patients on rounds at '+escapeHtml(roomCampus)+'.<br>Add one from the yellow sheet to get started.';
      let emptyHtml = '<div class="empty"><div class="big">—</div><p>'+emptyText+'</p></div>';
    ...
    wardNames.forEach(w=>{
      ...
      groups[w].forEach(p=>{
        ...
        const glimpseText = glimpse || (isInactive ? 'Catheter placed · Not yet activated' : '');
        ...
        <div class="row clickable rmode-${mode}" data-id="${p.id}">
          ...
          <div class="row-main">
            ...
            ${glimpseText ? `<div class="p-glimpse ${glimpse?'':'p-glimpse-empty'}">${escapeHtml(glimpseText)}</div>` : ''}
          </div>
          <div class="row-right">
            <div class="rr-line1">
    ...
    roundsActivePatients().forEach(p=>{
      getDoseUnits(p).forEach(holder=>{
        ...
        units.push({
          ...
          ward: p.ward, bed: p.bed, name: p.name, hospitalNo: p.hospitalNo, location: p.location, apsDate: p.apsDate,
          holder
        });
      });
    });
  ...
  function renderBolus(){
    const allUnits = bolusUnits();
    const wards = [...new Set(allUnits.map(u=>(u.ward||'Unassigned').trim() || 'Unassigned'))].sort((a,b)=>a.localeCompare(b));
    if(bolusWardFilter!=='all' && !wards.includes(bolusWardFilter)) bolusWardFilter = 'all';
    const wardSelect = $('bolusWardFilter');
    wardSelect.innerHTML = `<option value="all">All wards</option>${wards.map(w=>`<option value="${escapeHtml(w)}">${escapeHtml(w)}</option>`).join('')}`;
    wardSelect.value = bolusWardFilter;
    $('bolusDateFilter').value = bolusDateFilter;
    $('clearBolusDateFilter').textContent = bolusDateFilter ? 'Clear' : 'All dates';
    const list = allUnits.filter(u=>
      (bolusWardFilter==='all' || ((u.ward||'Unassigned').trim() || 'Unassigned')===bolusWardFilter) &&
      (!bolusDateFilter || u.apsDate===bolusDateFilter)
    );
    $('bolusCount').textContent = list.length;
    if(!list.length){
      const emptyText = allUnits.length ? 'No patients match the selected filters.' : 'No patients on Q4H / Q6H / Q8H / TID / Q12H interval dosing at '+escapeHtml(roomCampus)+' right now.';
      $('bolusBoard').innerHTML = '<div class="empty"><div class="big">—</div><p>'+emptyText+'</p></div>';
      return;
    }
    ...
    wardNames.forEach(w=>{
      ...
      groups[w].forEach(u=>{
        ...
        const location = u.label || u.location || '—';
        html += `
        <div class="row bolus-${status}" data-bolus-id="${unitKey}">
          <div class="row-main">
            <div class="row-top"><span class="p-name">${escapeHtml(u.name||'—')}</span><strong class="bolus-bed">Bed ${escapeHtml(u.bed||'—')}</strong></div>
            <div class="p-sub bolus-context">${escapeHtml(location)} · ${escapeHtml(u.holder.drug||'—')} · ${escapeHtml(u.holder.frequency||'—')}</div>
            <div class="p-sub">Hospital No. ${escapeHtml(u.hospitalNo||'—')}</div>
            <div class="p-sub">APS date: ${escapeHtml(fmtDDMMYY(u.apsDate))}</div>
          ...
          <div class="bolus-actions">
            <button class="bolus-frequency" data-patient-id="${u.patientId}" data-cath-id="${u.catheterId||''}">${escapeHtml(u.holder.freqType)}</button>
        ...
        try{
          await savePatientField(patientId, (base)=>{
            ...
            if(cathId){
              ...
              addDayActivity(c, 'bolus', bolusEntry);
            } else {
              base.boluses = base.boluses || [];
              base.boluses.push(bolusEntry);
              addDayActivity(base, 'bolus', bolusEntry);
            }
            logActivity(base, 'bolusLogged', {catheterId:cathId, bolusAt:bolusEntry.ts});
          });
        }catch(e){
    ...
    document.querySelectorAll('#bolusBoard .bolus-frequency').forEach(btn=>{
      btn.addEventListener('click', async ()=>{
        const patientId = btn.getAttribute('data-patient-id');
        const cathId = btn.getAttribute('data-cath-id') || null;
        const p = patients[patientId];
        ...
        if(!holder) return;
        const displayName = cathId ? `${p.name||'—'} — ${holder.label||'Catheter'}` : (p.name||'—');
        const next = await askBolusScheduleChange({name:displayName, current:holder.freqType});
        if(!next || next===holder.freqType) return;
        const changedAt = Date.now();
        const change = {ts:changedAt, from:holder.freqType||'—', to:next, by:(window.doloraUser&&window.doloraUser.name)||'Unknown', byId:(window.doloraUser&&window.doloraUser.id)||null};
        try{
          await savePatientField(patientId, base=>{
            ...
            target.freqType = next;
            target.startedAt = hospitalTimeNow();
            target.startedAtStamp = todayStr();
            target.scheduleChangedAt = changedAt;
            const detailDay = latestDetailDayIndex(target);
            target['day'+detailDay] = {...getDayRaw(target, detailDay), freqType:next, startedAt:target.startedAt};
            target.scheduleChanges = target.scheduleChanges || [];
            target.scheduleChanges.push(change);
            addDayActivity(target, 'schedule', change);
            if(cathId) syncCatheterDoseMirror(target);
            syncCurrentDoseMirror(base);
            logActivity(base, 'bolusFrequencyChanged', {catheterId:cathId, from:change.from, to:next, changedAt});
          });
        }catch(e){
          if(e && e.doloraDeleted){ handleDeletedElsewhere(patientId); return; }
        ...
        toast('Frequency changed to '+next+' for '+displayName);
        renderBolus();
      });
    });
  }
    ...
    wardNames.forEach(w=>{
      ...
      groups[w].forEach(r=>{
        ...
        rows += `<tr class="clickable" data-id="${p.id}" style="cursor:pointer;">
          ...
          <td>${eagleStatusLabel(p)}${(p.status==='discharged' && (p.dischargedDate||p.dischargedAt)) ? `<span style="display:block;font-size:10.5px;color:var(--ink-faint);margin-top:2px;">${p.dischargedDate ? fmtDDMMYY(p.dischargedDate) : fmtDDMMYYFromTs(p.dischargedAt)}</span>` : ''}</td>
        </tr>`;
      });
    });
    ...
    list.forEach(p=>{
      exportDoseUnits(p).forEach(u=>{
        for(let d=0; d<=5; d++){
          ...
          if(!dObj.stamp && !(dObj.bolusEvents||[]).length && !(dObj.scheduleChanges||[]).length) continue;
          reviewRows.push({
            'Hospital No': p.hospitalNo, 'Name': p.name, 'Campus': p.campus||'Main Campus', 'Ward': p.ward, 'Bed': p.bed,
            ...
            'Bolus Activity': [
              ...(dObj.scheduleChanges||[]).map(e=>`Schedule ${e.from||'—'} → ${e.to||'—'} at ${new Date(e.ts).toLocaleString()} by ${e.by||'Unknown'}`),
              ...(dObj.bolusEvents||[]).map(e=>`Bolus at ${new Date(e.ts).toLocaleString()} by ${e.by||'Unknown'}`)
            ].join('; '),
  ...
  function switchView(name){
    ...
    if(isRoomAlias){
      const nextRoomCampus = (name==='room-ranipet') ? 'Ranipet' : 'Main Campus';
      if(nextRoomCampus !== roomCampus){
        roundsWardFilter = 'all';
        roundsModeFilter = 'all';
        bolusWardFilter = 'all';
        bolusDateFilter = '';
      }
      roomCampus = nextRoomCampus;
  ...
  $('roundsWardFilter').addEventListener('change', e=>{
    roundsWardFilter = e.target.value;
    renderRounds();
  });
  $('roundsModeFilter').addEventListener('change', e=>{
    roundsModeFilter = e.target.value;
    renderRounds();
  });
  $('bolusWardFilter').addEventListener('change', e=>{
    bolusWardFilter = e.target.value;
    renderBolus();
  });
  $('bolusDateFilter').addEventListener('change', e=>{
    bolusDateFilter = e.target.value;
    renderBolus();
  });
  $('clearBolusDateFilter').addEventListener('click', ()=>{
    bolusDateFilter = '';
    renderBolus();
  });
  ...
  window.startDoloraApp = async function(){
    ...
    document.addEventListener('keydown', (e)=>{
      if(e.key === 'Escape'){
        ...
        } else if($('bolusScheduleBackdrop') && $('bolusScheduleBackdrop').classList.contains('open')){
          closeBolusScheduleModal(null);
