(function(){
  ...
  function latestReviewGlimpse(p){
    ...
    return `${latest.label ? latest.label+' · ' : ''}Day ${latest.day} · ${latest.remarks}`;
  }
    ...
    roundsActivePatients().filter(p=>patientMatchesRoomFilters(p, true)).forEach(p=>{
      getDoseUnits(p).forEach(holder=>{
        ...
        units.push({
          ...
          ward: p.ward, bed: p.bed, name: p.name, location: p.location,
          holder
        });
      });
    });
      ...
      groups[w].forEach(u=>{
        ...
        <div class="row bolus-${status}" data-bolus-id="${unitKey}">
          <div class="row-main">
            ...
            <div class="bolus-aps-date">APS date: ${escapeHtml(fmtDDMMYY(p.apsDate))}</div>
