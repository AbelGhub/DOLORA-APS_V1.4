(function(){
  ...
  // Filters deliberately live per campus, so narrowing Main Campus never
  // hides a patient when the user moves to Ranipet (or the reverse).
  let roomFiltersByCampus = (function(){
    try{ return JSON.parse(localStorage.getItem('aps-room-filters') || '{}'); }catch(e){ return {}; }
  })();
  ...
  // A Hospital No can legitimately have more than one APS episode over time
  // (e.g. an Epidural discharged, then a later PCA admission reuses the same
  // number). p.hospitalNo stays a plain, unsuffixed field on every record —
  // this is purely about giving each episode's *storage key* somewhere to
  // live without colliding with an earlier discharged one. The base record
  // for a number keeps the original unsuffixed key (patientKey(hospitalNo));
  // every episode after that gets base+'#2', base+'#3', etc.
  function findEpisodesByHospitalNo(hospitalNo){
    const base = patientKey(hospitalNo);
    if(!base) return [];
    return Object.values(patients).filter(p=>p && p.status!=='deleted' && patientKey(p.hospitalNo)===base);
  }
  function nextEpisodeKey(hospitalNo){
    const base = patientKey(hospitalNo);
    let maxN = 1;
    findEpisodesByHospitalNo(hospitalNo).forEach(p=>{
      if(p.id===base) return;
      const m = /#(\d+)$/.exec(p.id);
  ...
  function logActivity(p, action, meta){
    p.activityLog = p.activityLog || [];
    p.activityLog.push({
      ts: Date.now(),
      action, // 'edited' | 'discharged' | 'reactivated' | 'hospitalNoCorrected'
  ...
  async function postTeamUpdate(){
    ...
    renderTeamUpdates();
    markUpdatesSeen(roomCampus); // posting means you're caught up on this campus's feed
    toast('Posted to '+roomCampus);
  }

  // ---------- Updates unread badge ----------
  // Purely local, per-device "how far has this person scrolled" marker —
  // not synced, so each teammate's badge reflects their own unread state,
  // never someone else's.
  function updatesSeenKey(campus){ return 'aps-updates-seen:'+campus; }
  function getUpdatesSeenTs(campus){
    try{ return Number(localStorage.getItem(updatesSeenKey(campus))) || 0; }catch(e){ return 0; }
  }
  function markUpdatesSeen(campus){
    try{ localStorage.setItem(updatesSeenKey(campus), String(Date.now())); }catch(e){}
    updateUnreadBadge();
  }
  // Never counts the viewer's own posts as "new" — only teammates' posts
  // made since this device last opened the Updates tab for this campus.
  function unreadUpdatesCount(){
    const seenTs = getUpdatesSeenTs(roomCampus);
    const myId = (window.doloraUser && window.doloraUser.id) || null;
    return teamUpdates.filter(u=> u.ts > seenTs && u.byId !== myId).length;
  }
  function updateUnreadBadge(){
    const el = $('updatesBadge');
    if(!el) return;
    const n = unreadUpdatesCount();
    if(n>0){
  ...
  // Called after every teamUpdates load: if the person is actively looking
  // at the feed, show it and clear the badge; otherwise just refresh the
  // count so the badge stays live while they're on Rounds/Bolus.
  function refreshTeamUpdatesUI(){
    if(roomSubTab==='updates'){
      renderTeamUpdates();
      markUpdatesSeen(roomCampus);
    } else {
      updateUnreadBadge();
    }
  }
  ...
  // Compact calendar label for the Daily APS Review tabs. Kept separate from
  // fmtDDMMYY because tabs need no year and should remain easy to scan.
  ...
  // Rounds should carry the latest useful review forward, rather than only
  // showing a note saved today. Empty day sheets are deliberately skipped so
  // an earlier meaningful remark remains visible until it is superseded by a
  // later non-empty one. NBC reviews live on catheters, not on the patient,
  // so they are considered independently here as well.
  function latestReviewGlimpse(p){
    const holders = p.mode==='NBC' ? activeCatheters(p) : [p];
    const candidates = [];
    holders.forEach((holder, holderIdx)=>{
      for(let d=0; d<=5; d++){
        const remarks = (getDayRaw(holder, d).remarks || '').trim();
        if(!remarks) continue;
        candidates.push({
          day:d,
          remarks,
          label:p.mode==='NBC' ? ((holder.label&&holder.label.trim()) || ('Catheter '+(holderIdx+1))) : ''
        });
      }
    });
    if(!candidates.length) return '';
    // A higher APS day is the more clinically recent review. For two catheter
    // notes on the same day, preserve their stored order consistently.
    candidates.sort((a,b)=>b.day-a.day);
    const latest = candidates[0];
    // Show only what was actually typed into the remarks box — no "Day X"
    // or catheter-label prefix cluttering the rounds row.
  ...
  function dosingScheduleSelect(id, freqType){
    return `<select id="${id}">
      ...
      <option value="PIB Pump Q4H" ${freqType==='PIB Pump Q4H'?'selected':''}>PIB Pump Q4H</option>
      <option value="PIB Pump Q6H" ${freqType==='PIB Pump Q6H'?'selected':''}>PIB Pump Q6H</option>
      <option value="Q4H" ${freqType==='Q4H'?'selected':''}>Q4H</option>
      <option value="Q6H" ${freqType==='Q6H'?'selected':''}>Q6H</option>
      <option value="TID" ${freqType==='TID'?'selected':''}>TID</option>
  ...
  function renderDoseAndAssessmentFields(suf, eff, dObj, activeDay, holder){
    ...
    return `
      ...
      <div class="full-row" style="background:var(--surface-soft);border-radius:8px;padding:10px 12px;">
        <div class="row2">
          <div><label>Drug</label><input type="text" id="dr-drug${suf}" list="drug-suggestions" value="${escapeHtml(eff.drug)}"></div>
  ...
  function renderDailyRoundsBlock(p, activeDay){
    ...
    const tabs = [0,1,2,3,4,5].map(d=>
      `<button type="button" class="day-tab ${d===activeDay?'active':''}" data-day="${d}"><span>Day ${d}${dayHasStamp(d)?' ✓':''}</span><small>${escapeHtml(fmtDDMM(dayCalendarDate(p.apsDate, d)))}</small></button>`
    ).join('');
  ...
  // TID and Q8H intentionally share the same rolling eight-hour calculation
  // while preserving the originally documented schedule label.
  const BOLUS_INTERVAL_HOURS = {'Q4H':4, 'Q6H':6, 'TID':8, 'Q8H':8, 'Q12H':12};
  ...
  // typing, before anything is saved. Only meaningful for manual interval
  // schedules (Q4H/Q6H/TID/Q8H/Q12H) — other
  ...
  function currentBolusFrequencyHistory(holder){
    return (holder && holder.frequencyChanges) || [];
  }
  function fmtFrequencyChange(change){
    const from = change.from || '—';
    const to = change.to || '—';
    const by = change.by ? ' · '+change.by : '';
    return `Frequency changed ${from} → ${to}${by}`;
  }
  // Frequency changes take effect immediately for schedule calculation, while
  // preserving already-recorded review days. Future calendar days are updated
  // so their prefilled review fields match the active treatment schedule.
  function applyFrequencyChange(holder, apsDate, nextFrequency, entry){
    holder.freqType = nextFrequency;
    holder.frequencyChanges = holder.frequencyChanges || [];
    holder.frequencyChanges.push(entry);
    for(let d=0; d<=5; d++){
      const date = dayCalendarDate(apsDate, d);
      if(date && date >= todayStr() && holder['day'+d] && typeof holder['day'+d] === 'object'){
        holder['day'+d].freqType = nextFrequency;
      }
    }
  }
  ...
  // a warning before.
  let dcResolve = null;
  let dcPickedDischargeDate = null; // set only when this confirm showed the discharge-date picker
  function askConfirm({title, message, confirmLabel, danger, dischargeDate, defaultDate}){
    return new Promise(resolve=>{
      dcResolve = resolve;
      dcPickedDischargeDate = null;
      ...
      const showDate = !!dischargeDate;
      $('dcDischargeDateRow').style.display = showDate ? 'block' : 'none';
      if(showDate){
        // Defaults to today but is a normal editable date input, so a
        // clinician backdating a discharge logged after the fact can just
        // change it before confirming. Capped at today — a discharge can't
        // be recorded for a future date.
        const today = todayStr();
        const dateInput = $('dcDischargeDate');
        dateInput.max = today;
  ...
  $('dcConfirm').addEventListener('click', ()=>{
    if($('dcDischargeDateRow').style.display !== 'none'){
      dcPickedDischargeDate = $('dcDischargeDate').value || todayStr();
    }
    closeConfirmModal(true);
  });

  let frequencyChangeResolve = null;
  function askBolusFrequencyChange(currentFrequency){
    return new Promise(resolve=>{
      frequencyChangeResolve = resolve;
      $('frequencyNewValue').value = BOLUS_INTERVAL_HOURS[currentFrequency] ? currentFrequency : 'Q6H';
      $('frequencyBackdrop').classList.add('open');
      setTimeout(()=>{ try{ $('frequencyNewValue').focus(); }catch(e){} }, 50);
    });
  }
  function closeBolusFrequencyChange(value){
    $('frequencyBackdrop').classList.remove('open');
    if(frequencyChangeResolve){ const resolve = frequencyChangeResolve; frequencyChangeResolve = null; resolve(value); }
  }
  $('frequencyCancel').addEventListener('click', ()=>closeBolusFrequencyChange(null));
  $('frequencySave').addEventListener('click', ()=>closeBolusFrequencyChange($('frequencyNewValue').value));
  $('frequencyBackdrop').addEventListener('click', e=>{ if(e.target.id==='frequencyBackdrop') closeBolusFrequencyChange(null); });

  // ---------- Hospital No reuse popup (Add patient, discharged match) ----------
  // Entering a Hospital No that only matches discharged record(s) is
  // deliberately NOT auto-resolved either way: silently always reactivating
  // would wrongly merge a genuine new admission into an old episode, and
  // silently always creating a new record would wrongly split a "discharged
  // too early" mistake into two records for one continuous stay. So this
  // asks, once, at the moment it matters.
  let reuseResolve = null;
  function askHospitalReuse({hnoRaw, record, extraCount}){
    return new Promise(resolve=>{
      reuseResolve = resolve;
      const modeLabel = record.mode || 'Epidural';
      const when = record.dischargedAt ? fmtShortDateFromTs(record.dischargedAt) : (record.apsDate || '');
      $('reuseTitle').textContent = 'Hospital No '+hnoRaw+' — discharged record found';
      let msg = (record.name||'A patient')+' · '+modeLabel+' was discharged'+(when ? (' on '+when) : '')+'. Start a new APS entry, or reactivate that record?';
      if(extraCount>0) msg += ' ('+extraCount+' earlier discharged episode'+(extraCount>1?'s':'')+' for this Hospital No also exist'+(extraCount>1?'':'s')+'.)';
      $('reuseMsg').textContent = msg;
      $('reuseBackdrop').classList.add('open');
    });
  }
  function closeReuseModal(result){
    $('reuseBackdrop').classList.remove('open');
    if(reuseResolve){ const r = reuseResolve; reuseResolve = null; r(result); }
  }
  $('reuseNewBtn').addEventListener('click', ()=>closeReuseModal('new'));
  $('reuseReactivateBtn').addEventListener('click', ()=>closeReuseModal('reactivate'));
  $('reuseCancelBtn').addEventListener('click', ()=>closeReuseModal('cancel'));
  $('reuseBackdrop').addEventListener('click', (e)=>{ if(e.target.id==='reuseBackdrop') closeReuseModal('cancel'); });

  // ---------- Hospital No correction — "what should it be?" input popup ----------
  // Resolves to the trimmed new value the user typed, or null if they
  // cancelled. Validation (empty / unchanged / duplicate) happens in
  // correctHospitalNoFlow, not here — this popup only collects the input.
  let hnoInputResolve = null;
  function askNewHospitalNo(currentHno){
    return new Promise(resolve=>{
      hnoInputResolve = resolve;
      $('hnoCurrent').textContent = currentHno || '—';
      $('hnoNewInput').value = '';
      $('hnoBackdrop').classList.add('open');
      setTimeout(()=>{ try{ $('hnoNewInput').focus(); }catch(e){} }, 50);
    });
  }
  function closeHnoInputModal(result){
    $('hnoBackdrop').classList.remove('open');
    if(hnoInputResolve){ const r = hnoInputResolve; hnoInputResolve = null; r(result); }
  }
  $('hnoCancel').addEventListener('click', ()=>closeHnoInputModal(null));
  $('hnoBackdrop').addEventListener('click', (e)=>{ if(e.target.id==='hnoBackdrop') closeHnoInputModal(null); });
  $('hnoContinue').addEventListener('click', ()=>closeHnoInputModal($('hnoNewInput').value.trim()));
  $('hnoNewInput').addEventListener('keydown', (e)=>{ if(e.key==='Enter') closeHnoInputModal($('hnoNewInput').value.trim()); });

  // Shared by the Details-sheet discharge/reactivate toggle and the Add-form
  // hospital-no-reuse prompt, so both stay in sync.
  async function reactivatePatient(id){
    try{
      ...
      if(e && e.doloraDeleted){ handleDeletedElsewhere(id); return false; }
  ...
  async function openDischargeConfirm(id){
    ...
    if(!ok) return;
    // Noon anchor avoids the picked calendar date shifting a day under
    // timezone conversion — same convention as dayCalendarDate elsewhere.
    const dischargeDateStr = dcPickedDischargeDate || todayStr();
    const dischargedAtTs = new Date(dischargeDateStr+'T12:00:00').getTime();
    try{
      await savePatientField(id, (base)=>{
        base.status = 'discharged';
        base.dischargedAt = dischargedAtTs;
    ...
    toast('Marked discharged'+(dischargeDateStr!==todayStr() ? ' ('+fmtDDMMYY(dischargeDateStr)+')' : ''));
  ...
  // ---------- Hospital No correction (safe rename of the record's identity) ----------
  // Hospital No IS the storage key (patientKey(hospitalNo) === p.id), so a
  // typo can never be a normal field edit — it has to migrate the whole
  // record to a new key without ever risking the original. Order matters:
  // validate -> confirm -> write the NEW key -> verify what was written ->
  // only THEN delete the OLD key -> audit. If anything before "delete the
  // OLD key" fails, the original record is left completely untouched.
  async function correctHospitalNoFlow(oldId){
    const original = patients[oldId];
    if(!original) return;

    const newRaw = await askNewHospitalNo(original.hospitalNo || '');
    if(newRaw === null) return; // cancelled
    if(!newRaw){ toast('Enter a Hospital No.'); return; }

    const newKey = patientKey(newRaw);
    const currentKeyOfHno = patientKey(original.hospitalNo || '');
    if(newKey === currentKeyOfHno){
      toast('New Hospital No. must be different from the current one.');
      return;
    }

    // Any existing episode (active OR discharged) already using the target
    // number is treated as a conflict and blocks the correction outright —
    // this feature only ever fixes a typo into a number nobody else is
    // using. It deliberately does NOT reproduce the separate "reuse a
    // discharged Hospital No" prompt that Add Patient has; that's a
    // different workflow and mixing it in here risks merging two records.
    const conflicts = findEpisodesByHospitalNo(newRaw).filter(p2=>p2 && p2.id!==oldId);
    if(conflicts.length){
      const c = conflicts[0];
      toast('Hospital No '+newRaw+' is already in use by '+(c.name||'another patient')+' ('+(c.status==='discharged'?'discharged':'active')+') — choose a different number.', 6000);
      return;
    }

    if(original.status === 'discharged'){
      const proceedDischarged = await askConfirm({
        title: 'Patient already discharged',
        message: 'This patient has already been discharged. You are correcting the Hospital No. of a completed APS record. Continue?',
        confirmLabel: 'Continue'
      });
      if(!proceedDischarged) return;
    }

    const finalOk = await askConfirm({
      title: 'Correct Hospital No.?',
      message: (original.hospitalNo||'—')+' → '+newRaw+'. This will correct the patient\'s Hospital No. while preserving the complete existing APS record and history.',
      confirmLabel: 'Confirm Correction'
    });
    if(!finalOk) return;

    // Guard against a concurrent edit/delete before migrating anything —
    // same check used before every other destructive-ish action.
    const conflict = await hasConflict(oldId, patientUpdatedAt[oldId]);
    if(conflict === 'deleted'){ handleDeletedElsewhere(oldId); return; }
    if(conflict === 'edited'){
      toast('This record was updated elsewhere — reload and try the correction again.', 5000);
      return;
    }

    // Read the complete, current SERVER copy (not the possibly-stale
    // in-memory one) so nothing saved moments ago gets left behind.
    let freshRaw;
    try{
      const r = await window.storage.get('patient:'+oldId, true);
      freshRaw = r.value;
    }catch(e){
      toast('Could not read the current record — nothing changed. Try again ('+errMsg(e)+')', 5000);
      return;
    }
    let newObj;
    try{
      newObj = JSON.parse(freshRaw); // full clone straight from the stored JSON — every field, known or not, comes along untouched
    }catch(e){
      toast('Could not read the current record correctly — correction aborted, nothing changed.', 6000);
      return;
    }
    const oldHospitalNo = newObj.hospitalNo;
    const oldFullId = newObj.id;

    // Only the identity fields change — everything else is the exact object just read.
    newObj.id = newKey;
    newObj.hospitalNo = newRaw;

    // Write the NEW record first. The old one is not touched yet.
    try{
      await savePatient(newObj);
    }catch(e){
      toast('Correction failed — could not save the corrected record. Nothing was changed ('+errMsg(e)+')', 6000);
      return;
    }

    // Verify what actually landed on the server before trusting it.
    try{
      const verifyRes = await window.storage.get('patient:'+newKey, true);
      const verifyObj = JSON.parse(verifyRes.value);
      const intact = verifyObj && verifyObj.id===newKey && verifyObj.hospitalNo===newRaw
        && JSON.stringify(verifyObj.day0||null)===JSON.stringify(newObj.day0||null)
        && (verifyObj.boluses||[]).length === (newObj.boluses||[]).length
        && JSON.stringify(verifyObj.catheters||null)===JSON.stringify(newObj.catheters||null);
      if(!intact) throw new Error('verification mismatch');
    }catch(e){
      // Don't trust it — best-effort cleanup of the new key. The OLD record
      // is still sitting completely untouched under its original key.
      try{ await window.storage.delete('patient:'+newKey, true); }catch(e2){}
      toast('Correction could not be verified — nothing was changed. Please try again.', 6000);
      return;
    }

    // Only now remove the old identity.
    try{
      await window.storage.delete('patient:'+oldFullId, true);
    }catch(e){
      // The corrected record is already safely saved and verified — this
      // only means the old key is left behind as a stale duplicate.
      // Surfaced clearly rather than pretending everything is clean.
      toast('Corrected to '+newRaw+', but the old record ('+oldHospitalNo+') could not be removed — please delete it manually from Rounds.', 8000);
    }

    // Audit trail: on the record itself (existing activityLog mechanism)...
    logActivity(newObj, 'hospitalNoCorrected', {from: oldHospitalNo, to: newRaw});
    try{ await savePatient(newObj); }catch(e){ /* audit entry is best-effort; the correction itself already succeeded */ }
    // ...and a durable standalone entry, same pattern as recordDeletion(),
    // so the old->new relationship survives even once the old key is gone.
    try{
      await window.storage.set('correction-log:'+newKey+':'+Date.now(), JSON.stringify({
        type: 'hospitalNoCorrection',
        oldHospitalNo, newHospitalNo: newRaw, oldId: oldFullId, newId: newKey,
        name: newObj.name, ward: newObj.ward,
        by: (window.doloraUser && window.doloraUser.name) || 'Unknown',
        byId: (window.doloraUser && window.doloraUser.id) || null,
        ts: Date.now()
      }), true);
    }catch(e){ /* best-effort audit; correction already succeeded */ }

    // Swap local state and refresh the UI.
    delete patients[oldFullId];
    delete patientUpdatedAt[oldFullId];
    patients[newKey] = newObj;
    toast('Hospital No corrected: '+oldHospitalNo+' → '+newRaw);
    closeDetail();
    renderAll();
    openDetail(newKey);
  }
  ...
  function roomFilter(){
    if(!roomFiltersByCampus[roomCampus]) roomFiltersByCampus[roomCampus] = {ward:'all', mode:'all', apsDate:''};
    return roomFiltersByCampus[roomCampus];
  }
  function saveRoomFilters(){
    try{ localStorage.setItem('aps-room-filters', JSON.stringify(roomFiltersByCampus)); }catch(e){}
  }
  function resetRoomFilters(){
    roomFiltersByCampus[roomCampus] = {ward:'all', mode:'all', apsDate:''};
    saveRoomFilters();
  }
  function patientMatchesRoomFilters(p, includeApsDate){
    const filter = roomFilter();
    const ward = (p.ward && p.ward.trim()) ? p.ward.trim() : 'Unassigned';
    return (filter.ward==='all' || ward===filter.ward)
      && (filter.mode==='all' || (p.mode||'Epidural')===filter.mode)
      && (!includeApsDate || !filter.apsDate || p.apsDate===filter.apsDate);
  }
  function renderRoomFilters(){
    const filter = roomFilter();
    const wards = [...new Set(roundsActivePatients().map(p=>(p.ward&&p.ward.trim()) ? p.ward.trim() : 'Unassigned'))].sort(naturalSort);
    if(filter.ward!=='all' && !wards.includes(filter.ward)) filter.ward = 'all';
    $('roomWardFilter').innerHTML = '<option value="all">All wards</option>'+wards.map(w=>`<option value="${escapeHtml(w)}">${escapeHtml(w)}</option>`).join('');
    $('roomWardFilter').value = filter.ward;
    $('roomModeFilter').value = filter.mode;
    const isBolus = roomSubTab==='bolus';
    $('roomModeFilter').style.display = isBolus ? 'none' : '';
    $('roomApsDateButton').style.display = isBolus ? '' : 'none';
    $('roomApsDateButton').textContent = filter.apsDate ? ('APS date: '+fmtDDMMYY(filter.apsDate)) : 'All dates';
    $('roomApsDateFilter').value = filter.apsDate || '';
    if(isBolus){
      // Bolus only cares about patients started recently — cap the native
      // picker to [today-3, today] so it can't be pointed at stale dates.
      const today = todayStr();
      const minDate = dayCalendarDate(today, -3);
      $('roomApsDateFilter').min = minDate;
      $('roomApsDateFilter').max = today;
      // If a previously-picked date now falls outside the window (e.g. the
      // app sat open across midnight), drop back to "All dates" rather than
      // silently keeping a filter the picker itself would no longer allow.
      if(filter.apsDate && (filter.apsDate < minDate || filter.apsDate > today)){
        filter.apsDate = '';
        saveRoomFilters();
        $('roomApsDateButton').textContent = 'All dates';
        $('roomApsDateFilter').value = '';
      }
    }
    $('roomFilters').style.display = roomSubTab==='updates' ? 'none' : 'flex';
  }
  function renderRounds(){
    const active = roundsActivePatients().filter(patientMatchesRoomFilters);
    ...
    wardNames.forEach(w=>{
      ...
      groups[w].forEach(p=>{
        ...
        const glimpse = latestReviewGlimpse(p);
        ...
        // NBC's badge carries its last-saved dosing schedule alongside the
        // mode name (p.freqType mirrors the first active catheter — see
        // syncCurrentDoseMirror — which covers it for the common case of
        // both catheters sharing one schedule).
        const modeBadgeText = (mode==='NBC' && p.freqType) ? `${escapeHtml(modeLabels[mode]||mode)} · ${escapeHtml(p.freqType)}` : escapeHtml(modeLabels[mode]||mode);
        html += `
        <div class="row clickable rmode-${mode}" data-id="${p.id}">
          ...
          <div class="row-main">
            ...
            ${glimpse ? `<div class="p-glimpse">${escapeHtml(glimpse)}</div>` : ''}
          </div>
          <div class="row-right">
            <div class="rr-line1">
              <span class="mode-badge mode-${mode}">${modeBadgeText}</span>
            ...
            <div class="rr-line2">
              <span class="rr-doneby">${escapeHtml(p.doneBy||'—')}</span>
  ...
  function renderFrequencyHistory(p){
    const units = getDoseUnits(p);
    const entries = [];
    units.forEach(holder=>{
      const label = p.mode==='NBC' ? ((holder.label&&holder.label.trim())||'Catheter') : '';
      currentBolusFrequencyHistory(holder).slice().reverse().forEach(change=>{
        entries.push(`<div>${label?`<b>${escapeHtml(label)}:</b> `:''}${escapeHtml(fmtFrequencyChange(change))} · ${escapeHtml(new Date(change.ts).toLocaleString())}</div>`);
      });
    });
    return entries.length ? `<div class="field-hint bolus-frequency-history"><b>Frequency change history</b>${entries.join('')}</div>` : '';
  }
  function openDetail(id){
    ...
    $('sheet').innerHTML = `
      ...
      <div class="card">
        ...
        <div class="row2">
          <div><label>Hospital No <span class="campus-badge" title="Hospital No is this record's key and can't be edited directly">LOCKED</span></label>
            <div style="display:flex;gap:6px;align-items:center;">
              <input type="text" id="ed-hno" value="${escapeHtml(p.hospitalNo||'')}" readonly style="background:var(--surface-soft);color:var(--ink-soft);flex:1;">
              <button type="button" class="btn small secondary" id="fixHnoBtn" style="flex-shrink:0;">Fix typo</button>
            </div>
          </div>
        ...
        <input type="text" id="ed-location" list="location-suggestions" value="${escapeHtml(p.location||'')}" placeholder="Level of epidural / nerve block site">

        <button type="button" class="btn secondary" id="saveDetailsBtn" style="margin-top:12px;">Save details</button>
        <div class="field-hint" style="margin-top:4px;">Saves patient info above (ward, bed, name, date, mode, location, etc.) right away — separate from the Daily APS Review save below.</div>

        <div style="font-size:11px;color:var(--ink-soft);font-weight:600;margin:16px 0 5px;">Current dose</div>
        <div class="field-hint">${p.mode==='NBC' ? activeCatheters(p).map(c=>{
          const label = (c.label&&c.label.trim()) ? c.label.trim() : 'Catheter';
          // The holder's top-level fields are the live schedule mirror. In
          // particular, a frequency changed from the Bolus board must show
          // here immediately even when an older Day 5 review exists.
          const eff = c;
    ...
    if($('fixHnoBtn')){
      $('fixHnoBtn').addEventListener('click', ()=> correctHospitalNoFlow(p.id));
    }
    ...
    $('saveDetailsBtn').addEventListener('click', async ()=>{
      // Patient-level/demographic fields only (Hospital No through Location,
      // including APS mode) — deliberately separate from the Daily APS
      // Review save below, so correcting a typo'd ward, date, or location
      // never requires pain scores to already be filled in for the day.
      // Doesn't touch p.dayN, p.catheters, activation, or anticoag — those
      // stay exclusively the bottom Save Day button's job.
    ...
    if($('dischargeBtn')){
      $('dischargeBtn').onclick = async ()=>{
        if(p.status==='discharged'){
          ...
          const ok = await reactivatePatient(p.id);
          if(!ok) return;
          toast('Reactivated');
          renderAll();
          closeDetail();
  ...
  $('submitBtn').addEventListener('click', async ()=>{
    ...
    const episodes = findEpisodesByHospitalNo(hnoRaw);
    const activeExisting = episodes.find(p=>p.status!=='discharged');
    if(activeExisting){
      // Same as before: a live (non-discharged) record on this Hospital No
      // is always a block — never want two open entries for one admission.
      toast('Hospital No '+hnoRaw+' already exists ('+(activeExisting.campus||'Main Campus')+') — opening that record', 3500);
      switchView(activeExisting.campus==='Ranipet' ? 'room-ranipet' : 'room-main');
      openDetail(activeExisting.id);
      return;
    }

    let key = patientKey(hnoRaw);
    const dischargedExisting = episodes
      .filter(p=>p.status==='discharged')
      .sort((a,b)=>(b.dischargedAt||b.createdAt||0)-(a.dischargedAt||a.createdAt||0));
    if(dischargedExisting.length){
      const mostRecent = dischargedExisting[0];
      const choice = await askHospitalReuse({hnoRaw, record: mostRecent, extraCount: dischargedExisting.length-1});
      if(choice === 'cancel') return;
      if(choice === 'reactivate'){
        const ok = await reactivatePatient(mostRecent.id);
        if(!ok) return;
        toast('Reactivated existing record');
        switchView(mostRecent.campus==='Ranipet' ? 'room-ranipet' : 'room-main');
        openDetail(mostRecent.id);
        renderAll();
        return;
      }
      // choice === 'new': this is a fresh episode on a number that already
      // has discharged history — give it its own storage key so it can't
      // collide with the discharged one(s). hospitalNo itself (set below,
      // from hnoRaw) stays exactly what was typed, unsuffixed, everywhere
      // it's displayed/exported/searched.
      key = nextEpisodeKey(hnoRaw);
    }
  ...
  function bolusUnits(){
    ...
    const units = [];
    roundsActivePatients().filter(p=>patientMatchesRoomFilters(p, true)).forEach(p=>{
      getDoseUnits(p).forEach(holder=>{
        if(!isBolusEligible(holder)) return;
        units.push({
          patientId: p.id,
          catheterId: p.mode==='NBC' ? holder.id : null,
          // Prefer this catheter's own label (its Location text). Legacy
          // single-catheter NBC records synthesize an empty label (see
          // getCatheters), so fall back to the patient-level location field
          // before ever falling back to the generic word "Catheter".
          label: p.mode==='NBC' ? ((holder.label&&holder.label.trim())||(p.location&&p.location.trim())||'Catheter') : null,
          ward: p.ward, bed: p.bed, name: p.name, location: p.location, apsDate: p.apsDate,
          holder
        });
      });
    });
  ...
  function renderBolus(){
    ...
    if(!list.length){
      $('bolusBoard').innerHTML = '<div class="empty"><div class="big">—</div><p>No patients on Q4H / Q6H / TID / Q8H / Q12H interval dosing at '+escapeHtml(roomCampus)+' right now.</p></div>';
      return;
    }
    ...
    wardNames.forEach(w=>{
      ...
      groups[w].forEach(u=>{
        ...
        const blockLocation = u.label || u.location || '—';
        html += `
        <div class="row bolus-${status}" data-bolus-id="${unitKey}">
          <div class="row-main">
            <div class="row-top"><span class="p-name">${escapeHtml(u.name||'—')}</span><span class="p-bed">Bed ${escapeHtml(u.bed||'—')}</span></div>
            <div class="p-sub">${escapeHtml(blockLocation)} · ${escapeHtml(u.holder.drug||'—')} · ${escapeHtml(u.holder.frequency||'—')}</div>
            <div class="bolus-aps-date">APS date: ${escapeHtml(fmtDDMMYY(u.apsDate))}</div>
          ...
          <div class="bolus-actions">
            <div class="bolus-frequency-control"><span>${escapeHtml(u.holder.freqType)}</span><button type="button" class="change-frequency" data-patient-id="${u.patientId}" data-cath-id="${u.catheterId||''}">Change</button></div>
    ...
    document.querySelectorAll('#bolusBoard .change-frequency').forEach(btn=>{
      btn.addEventListener('click', async ()=>{
        const patientId = btn.getAttribute('data-patient-id');
        const cathId = btn.getAttribute('data-cath-id') || null;
        const p = patients[patientId];
        if(!p) return;
        const holder = cathId ? activeCatheters(p).find(c=>c.id===cathId) : p;
        if(!holder) return;
        const nextFrequency = await askBolusFrequencyChange(holder.freqType);
        if(!nextFrequency || nextFrequency===holder.freqType) return;
        const fromFrequency = holder.freqType;
        const entry = {ts:Date.now(), from:fromFrequency, to:nextFrequency, by:(window.doloraUser&&window.doloraUser.name)||'Unknown', byId:(window.doloraUser&&window.doloraUser.id)||null, basis:'last-bolus'};
        try{
          await savePatientField(patientId, base=>{
            const target = cathId ? ensureCatheters(base).find(c=>c.id===cathId) : base;
            if(!target) throw new Error('catheter not found (may have been removed)');
            applyFrequencyChange(target, base.apsDate, nextFrequency, entry);
            if(!cathId) base.freqType = nextFrequency;
          });
        }catch(e){
          if(e && e.doloraDeleted){ handleDeletedElsewhere(patientId); return; }
          toast('Could not change frequency: '+errMsg(e), 5000);
          return;
        }
        toast(`Frequency changed to ${nextFrequency}; next bolus recalculated from the last logged bolus`);
        renderBolus();
      });
    });
  ...
  function applyRoomSubTabUI(){
    ...
    if($('roomFilters')) $('roomFilters').style.display = roomSubTab==='updates' ? 'none' : 'flex';
  }
  function switchView(name){
    ...
    if(isRoomAlias){
      ...
      if(roomSubTab==='bolus') resetRoomFilters();
      applyRoomSubTabUI();
      renderRoomFilters();
      renderRounds();
      renderBolus();
      loadTeamUpdates().then(refreshTeamUpdatesUI);
    } else {
  ...
  $('roomWardFilter').addEventListener('change', ()=>{
    roomFilter().ward = $('roomWardFilter').value;
    saveRoomFilters();
    renderRounds();
    renderBolus();
  });
  $('roomModeFilter').addEventListener('change', ()=>{
    roomFilter().mode = $('roomModeFilter').value;
    saveRoomFilters();
    renderRounds();
    renderBolus();
  });
  $('roomApsDateButton').addEventListener('click', ()=>{
    // A selected date turns this compact control into a one-tap return to
    // All dates. From the default state it opens the native calendar picker.
    if(roomFilter().apsDate){
      roomFilter().apsDate = '';
      saveRoomFilters();
      renderRoomFilters();
      renderBolus();
    ...
    const picker = $('roomApsDateFilter');
    if(picker.showPicker) picker.showPicker();
    else picker.click();
  });
  $('roomApsDateFilter').addEventListener('change', ()=>{
    roomFilter().apsDate = $('roomApsDateFilter').value;
    saveRoomFilters();
    renderRoomFilters();
    renderBolus();
  });
  ...
  function refreshAutocompleteLists(){
    const doneBy = new Set(), dx = new Set(), surgery = new Set(), location = new Set(), drug = new Set();
    Object.values(patients).forEach(p=>{
      ...
      if(p.location && p.location.trim()) location.add(p.location.trim());
      if(p.mode==='NBC'){
        // Each catheter's "label" IS its location text (see intake save),
        // and each carries its own current drug via syncCatheterDoseMirror —
        // both are worth suggesting, same as the patient-level fields above.
        getCatheters(p).forEach(c=>{
          if(c.label && c.label.trim()) location.add(c.label.trim());
          if(c.drug && c.drug.trim()) drug.add(c.drug.trim());
        });
      } else if(p.drug && p.drug.trim()){
        drug.add(p.drug.trim());
      }
    });
    const fill = (id, set) => {
      const el = $(id);
    ...
    fill('location-suggestions', location);
    fill('drug-suggestions', drug);
  }

  function renderAll(){
    renderDashboard();
    if(document.getElementById('view-room').classList.contains('active')) renderRoomFilters();
    renderRounds();
  ...
  window.startDoloraApp = async function(){
    ...
    applyRoomSubTabUI();
    await loadTeamUpdates();
    refreshTeamUpdatesUI();
    refreshWardOptions();
    if($('f-apsdate')) $('f-apsdate').value = todayStr();
    updateModeBadge('f-mode','f-mode-badge');
    renderAll();
    // Poll for teammates' new updates while a campus Room is open — keeps
    // the Updates feed (when it's the visible subtab) and the unread badge
    // (when it isn't) both live without the person needing to tap in.
    setInterval(async ()=>{
