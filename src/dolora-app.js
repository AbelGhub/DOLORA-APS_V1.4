  ...
  function openDetail(id){
    ...
    roundsBlock.addEventListener('click', async (e)=>{
      ...
      if(e.target.closest('#dr-attended-toggle')){
        ...
        const toggleBtn = $('dr-attended-toggle');
        const toggleCheck = $('dr-attended-check');
        if(toggleBtn) toggleBtn.classList.toggle('on', nowOn);
        if(toggleCheck){ toggleCheck.classList.toggle('on', nowOn); toggleCheck.textContent = nowOn ? '✓' : ''; }
        renderRounds();
        renderDashboard();
        return;
      }
