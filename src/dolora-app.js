(function(){
  ...
  async function hasConflict(id, openedAt){
    ...
    try{
      ...
      return (r && r.updated_at && r.updated_at !== openedAt) ? 'edited' : false;
    }catch(e){
  ...
  function todaysGlimpse(p){
    ...
    for(let d=5; d>=0; d--){
      ...
      if(dObj.stamp === today && dObj.remarks && dObj.remarks.trim()){
        return dObj.remarks.trim();
      }
    }
    return null;
  }
    ...
    return `
      ...
      <div class="day-review-grid" style="margin-top:14px;">
        <div class="full-row" style="background:var(--alert-bg);border:1.5px solid var(--alert);border-radius:8px;padding:10px 12px;">
          <div style="font-size:11px;font-weight:700;color:var(--alert);margin-bottom:6px;">Anticoag</div>
          <div class="row2">
        ...
        <div class="full-row review-footer-row">
          <button type="button" class="attend-toggle ${isAttendedToday(p)?'on':''}" id="dr-attended-toggle">
            <span class="attend-check ${isAttendedToday(p)?'on':''}" id="dr-attended-check">${isAttendedToday(p)?'✓':''}</span>
            <span class="attend-label">ATTENDED<span class="attend-sub">TODAY</span></span>
          </button>
          ${isNBC ? '' : `
    ...
    wardNames.forEach(w=>{
      ...
      groups[w].forEach(p=>{
        ...
        <div class="row clickable rmode-${mode}" data-id="${p.id}">
          <span class="attend-box ${seenToday?'checked':''}">${seenToday?'✓':''}</span>
          <div class="row-main">
            ...
            <div class="p-glimpse ${glimpse?'':'p-glimpse-empty'}">${glimpse?escapeHtml(glimpse):defaultGlimpse}</div>
          </div>
          <div class="row-right">
  ...
  function openDetail(id){
    const p = patients[id];
    if(!p) return;
    const sheetOpenedAt = patientUpdatedAt[id] || null; // baseline for conflict check on save
    ...
    $('sheet').innerHTML = `
      ...
      <div class="card">
        ...
        <div style="margin-bottom:8px;">
          ...
          <input type="text" id="ed-doneby" value="${escapeHtml(p.doneBy||'')}" placeholder="${(p.doneByRole==='surgeon') ? 'e.g. Dr. Mohan / Ortho Unit' : 'e.g. Dr. Anand'}">
        ...
        <label>Dx</label><input type="text" id="ed-dx" value="${escapeHtml(p.dx||'')}">
        <label>Surgery</label><input type="text" id="ed-surgery" value="${escapeHtml(p.surgery||'')}">
    ...
    roundsBlock.addEventListener('click', async (e)=>{
      ...
      if(e.target.closest('#dr-attended-toggle')){
        ...
        toast(nowOn ? 'Marked attended today' : 'Attendance unmarked');
        roundsBlock.innerHTML = renderDailyRoundsBlock(p, activeDay);
        wireAllBolusHints(p);
        renderRounds();
        renderDashboard();
  ...
  function dayCell(p, idx){
    ...
    const stampTag = d.stamp ? `<span style="color:var(--safe);font-family:var(--mono);font-size:10px;">✓${fmtShortDate(d.stamp)}</span> ` : '';
  ...
  function openTeamModal(){
    ...
    if($('teamModalInput')) $('teamModalInput').value = (cur && cur.team) ? cur.team : '';
  ...
  if($('teamModalSave')){
    $('teamModalSave').addEventListener('click', async ()=>{
      ...
      const singleVal = $('teamModalInput') ? $('teamModalInput').value.trim() : '';
      await saveMonthlyRoster(eagleMonth, {
        mainTeam: mainVal || singleVal,
        ranipetTeam: ranipetVal,
        team: singleVal || [mainVal, ranipetVal].filter(Boolean).join(', ')
      });
      closeTeamModal();
      ...
      exportDoseUnits(p).forEach(u=>{
        for(let d=0; d<=5; d++){
          ...
          reviewRows.push({
            ...
            'Day': d, 'Date': fmtDDMMYY(dObj.stamp),
  ...
  async function renderAdmin(){
      ...
      pendingListEl.innerHTML = pending.map(p => `
        <div class="card" style="margin-bottom:10px;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 14px;">
          ...
          <div style="display:flex;gap:6px;flex-shrink:0;">
            <button class="btn" style="padding:6px 12px;font-size:12px;margin:0;" data-approve-id="${p.id}" data-approve-role="member">Approve</button>
      ...
      memberListEl.innerHTML = members.map(m => {
        ...
        return `
          <div class="card" style="margin-bottom:8px;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 14px;">
            ...
            <div style="display:flex;align-items:center;gap:8px;">
              <span class="campus-badge" style="${m.role==='admin' ? 'background:var(--dark);color:#fff;border-color:var(--dark);' : ''}">${m.role==='admin' ? 'Admin' : 'Member'}</span>
              ${!isSelf ? `
    ...
    if($('adminRosterMonth')){
      ...
      loadMonthlyRoster(curYm).then(cur => {
        ...
        if($('adminRosterTeamText')) $('adminRosterTeamText').value = (cur && cur.team) ? cur.team : '';
      });
      ...
      $('adminLoadRosterBtn').onclick = async () => {
        ...
        if($('adminRosterTeamText')) $('adminRosterTeamText').value = (r && r.team) ? r.team : '';
      ...
      $('adminSaveRosterBtn').onclick = async () => {
        ...
        const singleText = ($('adminRosterTeamText') ? $('adminRosterTeamText').value : '').trim();
        await saveMonthlyRoster(selYm, {
          mainTeam: mainText || singleText,
          ranipetTeam: ranipetText,
          team: singleText || [mainText, ranipetText].filter(Boolean).join(', ')
        });
