(function(){
  ...
  async function routeForSession(session){
    ...
    try{
      ...
      if(!profile){
        // Profile row might not have been created yet; check if this is the first user
        try {
      ...
      if(!profile || profile.role === 'pending'){
        // If there are currently zero admins in the system, bootstrap this user as admin
        try {
          const { data: admins } = await sb.from('profiles').select('id').eq('role', 'admin').limit(1);
          if(!admins || admins.length === 0){
            await sb.from('profiles').update({ role: 'admin' }).eq('id', session.user.id);
            if(profile) profile.role = 'admin';
            else profile = { role: 'admin', full_name: session.user.email };
          }
        } catch(e){}
      }
